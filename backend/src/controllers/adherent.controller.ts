...
export const createAdherent = async (req: Request, res: Response) => {
    try {
        ...
        const newAdherent = new Adherent({ ...rest, mdp: hashedPassword });
...
export const updateAdherent = async (req: Request, res: Response) => {
    try {
        const { mdp, ...rest } = req.body;
        const dataToUpdate = mdp ? { ...rest, mdp: await bcrypt.hash(mdp, 10) } : rest;
        const updatedAdherent = await Adherent.findByIdAndUpdate(req.params.id, dataToUpdate, { new: true });
        if (!updatedAdherent) return res.status(404).json({ message: 'Adhérent non trouvé' });
...
export const deleteAdherent = async (req: Request, res: Response) => {
    try {
        ...
        if (!deletedAdherent) return res.status(404).json({ message: 'Adhérent non trouvé' });
...
export const loginAdherent = async (req: Request, res: Response) => {
    try {
        ...
        const token = jwt.sign(
            { id: adherent._id, nom: adherent.nom, prenom: adherent.prenom, email: adherent.email, role: adherent.role },
            JWT_SECRET,
            { expiresIn: '15m' }
        );
